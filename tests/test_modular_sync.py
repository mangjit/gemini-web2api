...
class StreamingEndpointTests(unittest.TestCase):
    ...
    def test_root_serves_playground(self):
        ...
        self.assertIn(b'id="saveCookie"', body)
        self.assertIn(b"GEMINI_COOKIE", body)
        self.assertIn(b"X-Gemini-Cookie", body)
        self.assertNotIn(b'id="googleSignIn"', body)
        self.assertNotIn(b"Sign in with Google", body)
        self.assertNotIn(b"AddSession", body)
        self.assertNotIn(b"request-cookies", body)
